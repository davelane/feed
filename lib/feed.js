...
function atom_1_0(options) {
    ...
    for(var i = 0; i < entries.length; i++) {
        ...
        // media
        if(entries[i].media) {
            for(var y = 0; y < entries[i].media.length; y++) {
                var entryMedia = {link: { _attr: { rel: 'enclosure' }}};

                if(entries[i].media[y].type)
                    entryMedia.link._attr.type = entries[i].media[y].type;

                if(entries[i].media[y].title)
                    entryMedia.link._attr.title = entries[i].media[y].title;

                if(entries[i].media[y].url)
                    entryMedia.link._attr.href = entries[i].media[y].url;

                if(entries[i].media[y].length)
                    entryMedia.link._attr.length = entries[i].media[y].length;
                    
                entry.push(entryMedia);
            }
        }
...
function rss_2_0(options) {
    ...
    for(var i = 0; i < items.length; i++) {
        ...
        // Keep backwards support for item.image but throw an exception if
        // item.image and image.media is set
        if(items[i].image && items[i].media)
            throw new Error('item cannot have image and media. rss allows only one enclosure');
            
        if(items[i].image) {
            bMediaRss = true;
            item.push({ 'media:thumbnail': [{ _attr: { url: items[i].image } }] });
        }

        if(items[i].media) {
            // Check to see if more that one media (rss allows only one, atom several)
            if(items[i].media.length > 1)
                throw new Error('rss allows only one enclosure');
                
            if(!items[i].media[0].url)
                throw new Error('media url must be set');
            if(!items[i].media[0].length)
                throw new Error('media length must be set');
            if(!items[i].media[0].type)
                throw new Error('media type must be set');
                
            item.push({ enclosure: [{ _attr: 
                {
                    url: items[i].media[0].url,
                    length: items[i].media[0].length,
                    type: items[i].media[0].type
                } 
            }]});
        }
